...
import { LOG_PREFIX, POLL_INTERVAL_MS } from "../constants"
...
export class ModelRefreshMonitor {
  ...
  private async poll(providerKey: string, baseUrl: string, notifier: Notifier): Promise<void> {
    try {
      ...
      await this.notifyChanges(providerKey, notifier, added, removed)
      this.knownModels.set(baseUrl, current)
    } catch {
  ...
  private async notifyChanges(
    ...
    for (const id of added) {
      console.info(`${LOG_PREFIX} New model "${id}" for provider "${providerKey}"`)
      await notifier.info(`New model "${id}" discovered for provider "${providerKey}"`)
    }
    for (const id of removed) {
      console.info(`${LOG_PREFIX} Model "${id}" removed from provider "${providerKey}"`)
      await notifier.warning(`Model "${id}" removed from provider "${providerKey}"`)
    }
  }
}
